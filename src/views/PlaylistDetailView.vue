<template>
  <div class="view playlist-detail">
    <section class="playlist-hero">
      ...
      <div class="playlist-hero__content">
        ...
        <div class="playlist-meta">
          ...
          <span v-if="playlist.tags?.length">{{ playlist.tags.join(' / ') }}</span>
        </div>
    ...
    <n-modal
      ...
      preset="card"
      class="playlist-comments-modal"
      :bordered="false"
      :mask-closable="true"
      transform-origin="center"
    >
      <template #header>
        <div class="playlist-comments-modal__title">
          <span>歌单评论</span>
          <small>{{ commentTotal }} 条</small>
        </div>
      </template>

      <div class="playlist-comments__scroll">
        <div v-if="commentsLoading && !comments.length" class="playlist-state">评论加载中...</div>
        <div v-else-if="commentsError" class="playlist-state playlist-state--error">{{ commentsError }}</div>

        <template v-else>
          <section v-if="hotComments.length" class="comment-block">
            <h3>热门评论</h3>
            <article v-for="comment in hotComments" :key="`hot-${comment.id}`" class="comment-item">
              <img class="comment-item__avatar" :src="comment.user.avatarUrl" :alt="comment.user.name" />
          ...
          <section class="comment-block">
            ...
            <div v-if="!comments.length" class="playlist-state">暂无评论</div>
            <button
              v-if="commentsHasMore"
              class="playlist-comments__more"
              type="button"
...
import { MessageCircle, Pause, Play, ThumbsUp } from 'lucide-vue-next'
import { useMessage } from 'naive-ui'
import SongListRow from '../components/SongListRow.vue'
import { getPlaylistCommentsData, getPlaylistDetailData } from '../services/netease'
...
const creatorInitial = computed(() => playlist.value.creator?.slice(0, 1) || '云')
